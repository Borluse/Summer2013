#pragma strict
@script RequireComponent(__p_uXforms)        ; //shares all player state and anim info for add-ons to use ..character controller state not accessible directly

class __pForce extends __pUnit2D{                          //Force commands

  protected var uXforms : Transform[]   ; //stores eligible targets

  private var _ux_    : __p_uXforms    ;
  ...
  if(!bStill){  //WTH does this do? Threshold for jump->grapple
    if (_io_.bJumpD) {
        ...
        if(animState != animEnum.dash){
          ...
          if(pointObj){
            if(_io_.bJump){             //hold to grapple
              ...
              //if(_io_.Get_kY()<0.1){  //grapple only on desecnt 
                holdBool         = true                                                                         ;
  ...
  function Get_UX():__p_uXforms{    
    var return_ux:__p_uXforms = xform.root.GetComponentInChildren(__p_uXforms) ;
    if(return_ux){
      print(this+"__pForce : Found __p_uXforms on xform and are attaching    : " + xform) ;
    }
    else{
      print("ERROR: "+this+"__pForce : Looking __p_uXforms on xform and also not found : ERROR IN STRUCTURE !" + xform);
    }
    return return_ux;
  }
